...
---
*Last updated: 2026-05-03 after Phase 16.4 SpacetimeDB v2.2.0 refactor pass. Adopted v2.2.0 surfaces opportunistically: `ctx.db.X.clear()` replaced 50 manual iter+delete sites in `server_nuke_test_data` (SC#1); `spacetime:publish:migrate` sibling script added (SC#3); v3 WebSocket transport baseline captured at `transport-evidence/` showing 796:1 egress dominance (SC#4); `useTable({ enabled })` adopted at admin panels (SC#5); `useReducer(reducers.X)` adopted at 5 React component sites (SC#6). Plan 02 `isInternal` guards on 3 scheduled reducers were reverted by Plan 07 hotfix after SDK source-level diagnostic revealed v2.2.0's `senderAuth.isInternal` is always false — engine-level "no such reducer" rejection is the operational defense (SC#2 audit value preserved as durable deliverable). Energy-model byte-identical pre/post (SC#8). No B-tree #4733 candidates found (SC#9). 41 commits across 7 plans. 10 code-review findings deferred to UAT evaluation per `deferred-items.md` (3 in-scope items fixed: WR-04 `loginAsGuest({})` no-arg adoption, WR-05 token-equality guard fail-open fix, WR-08 `useTable [rows, isReady]` tuple destructure). Next: Phase 17 (cost-tables data layer).*
