...
expected: |
  Call `update_tournament` on a Draft-stage tournament. Fields update successfully. Repeat in Registration stage — still works. Attempt on a Seeding/InProgress/Completed tournament — rejected with error.
awaiting: paused — bug fixed, re-test pending
...
result: issue
reported: "ensureTournamentAccess panics at line 49 — ctx.db.TournamentAssistant.primaryKey.find() crashes because .primaryKey is undefined on composite PK table handles. Server-side PANIC for any non-organizer, non-Moderator caller. Blocks update_tournament, advance_tournament_stage, cancel_tournament, and all reducers using ensureTournamentAccess."
severity: blocker
...
- truth: "update_tournament works for organizer in Draft/Registration stages"
  status: failed
  reason: "User reported: ensureTournamentAccess panics at line 49 — ctx.db.TournamentAssistant.primaryKey.find() crashes because .primaryKey is undefined on composite PK table handles. Server-side PANIC for any non-organizer, non-Moderator caller."
  severity: blocker
  test: 3
  root_cause: "tournamentHelpers.ts:49 uses (ctx.db.TournamentAssistant as any).primaryKey.find({...}) but SpacetimeDB TS SDK does not expose a .primaryKey accessor on composite PK tables. The property is undefined, causing .find() to throw TypeError."
  artifacts:
    - path: "spacetimedb/src/helpers/tournamentHelpers.ts"
      issue: "Line 49: .primaryKey.find() is not a valid SpacetimeDB table accessor for composite PK tables"
  missing:
    - "Replace .primaryKey.find() with index-based lookup (e.g., tournament_id.filter + manual .find()) or iter() filter"
  debug_session: ""
