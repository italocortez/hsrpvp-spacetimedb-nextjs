...
number: 2
name: Create Tournament with Bracket Fields
expected: |
  Calling create_tournament with groupSize, has3rdPlaceMatch, and autoAdvanceBracket parameters succeeds. The Tournament row reflects these new column values. Omitting them also works (defaults applied).
...
result: issue
reported: "test/shared/connection.ts:128 uses identityHex but generated binding expects callerIdentityHex — pre-existing mismatch, fixed inline"
severity: major
