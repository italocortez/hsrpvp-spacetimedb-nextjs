...
findings:
  ...
  info: 17
  total: 27
audit_notes: .planning/phases/16-route-global-foundation/16-REVIEW-NOTES.md
planned_fixes: 12
skipped: 8
---

> **Audit decisions:** see `16-REVIEW-NOTES.md` for skip/fix classification per finding.
> 🟢 = apply as-specified · 🟡 = apply with modifications · 🔴 = skip (reason in notes)
...
### WR-01: Stage 2 `subscribedRef` reset ordering in cleanup 🔴 SKIP
...
### WR-02: `dev-unregister-sw` crashes when `navigator.serviceWorker` or `caches` is undefined 🟢 FIX
...
### WR-03: Service Worker `cache.put` is fire-and-forget 🟡 FIX-MODIFIED (use `event.waitUntil`)
...
### WR-04: CSP allows `'unsafe-eval'` and `'unsafe-inline'` 🟡 FIX-MODIFIED (drop `'unsafe-eval'` only)
...
### WR-05: `resolvedSignatureRef` not cleared on identity/session change 🔴 SKIP
...
### WR-06: `LoadoutDropdown` dual click listeners 🟢 FIX
...
### WR-07: `GameDataProvider` `isReady` always truthy 🟢 FIX (highest-impact warning — real bug)
...
### WR-08: `SafariWarning` calls `isSafari()` three times on mount 🟢 FIX
...
### WR-09: `ViewportGate` creates `dynamic(...)` inside render body 🟢 FIX
...
### WR-10: Middleware redirect loop hazard if matcher extended to `/` 🟢 FIX
...
### IN-01: `any` types on SpacetimeDB subscription callbacks 🟢 FIX
...
### IN-02: Extensive `console.log` in production bundles 🔴 SKIP (user veto)
...
### IN-03: `GameDataProvider` uses `as unknown as` type laundering 🔴 SKIP (user veto)
...
### IN-04: `TeamSlot` has dead commented-out `<Image />` props 🟢 FIX
...
### IN-05: `TeamSlot` raw `<img>` with empty-string src fallback 🟡 FIX-MODIFIED (fallback asset + onError, all 5 `<img>` sites)
...
### IN-06: `LoadoutDropdown` portraits missing lazy loading 🟡 FIX-MODIFIED (decoding="async" only, skip lazy)
...
### IN-07: `useAuth` effect dep on `getConnection` without stability docs 🟢 FIX
...
### IN-08: `middleware.ts` matcher omits `/api/` — document intent 🟢 FIX
...
### IN-09: `SafariWarning` inline styles bypass design tokens 🔴 SKIP (user veto — invented token + wrong shade)
...
### IN-10: `dev-unregister-sw` inline styles ⚪ NO-OP (self-cancelled, intentional per D-25)
...
### IN-11: SW registration missing "update available" lifecycle 🟢 FIX
...
### IN-12: `useAuth` magic number for Discord intent TTL 🟢 FIX
...
### IN-13: `next.config.ts` CSP includes `ws://localhost:*` in production 🔴 SKIP (user veto — zero impact in dev-only workflow)
...
### IN-14: `(authed)/(match)/layout.tsx` is a passthrough placeholder 🟢 FIX (JSDoc)
...
### IN-15: `isLiveChange` helper duplicated across subscription owners 🟢 FIX
...
### IN-16: Component-hygiene doc's "enforced starting Phase 16" is ambiguous 🟢 FIX
...
### IN-17: `useAuth` Discord-flow effect has 7 early-return branches 🔴 SKIP (user veto — future work, high regression risk)
