...
**Goal**: Players can record which HSR characters and lightcones they own, manage multiple accounts, and control roster visibility â€” enforced at the server level
**Depends on**: Phase 1
...
**Plans:** 2 plans
Plans:
- [ ] 02-01-PLAN.md â€” Schema changes, new tables, helpers, user deletion cascade— Schema changes, new tables, helpers, user deletion cascade
- [ ] 02-02-PLAN.md â€” User and admin roster reducers, archetype CRUD, exports, docs, publish— User and admin roster reducers, archetype CRUD, exports, docs, publish
...
**Success Criteria** (what must be TRUE):
  1. A user can request the TO role via reducer; an admin can approve or deny the request
  2. A TO can create a tournament with format (single elimination, double elimination, group phase), game mode, and all tournament-level settings; tournament advances through explicit stage transitions (Draft â†’ Registration â†’ Seeding â†’ InProgress â†’ Completed/Cancelled)
  3. A player can sign up for a tournament individually; a team captain can sign up an entire team as a unit
  4. A TO can assign one or more users as referees scoped to that tournament; referees cannot act outside their assigned tournament
  5. Tournament-level settings (anonymous play default, open/closed roster, disconnect policy) are stored and readable by match reducers
...
**Success Criteria** (what must be TRUE):
  1. When a lobby or match has anonymous mode enabled, cursor events and match events carry an anonymousLabel (e.g., "Blue-1") instead of userId at the write layer â€” not just at display
