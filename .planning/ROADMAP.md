...
Plans:
- [x] 15.5-01-backend-cleanup-PLAN.md — Wave 1: delete view_user_directory, rename view_public_accounts → view_public_hsr_accounts, single-shot publish + two-dir spacetime generate, build gate (S-01, S-02, D-02, D-04)
- [x] 15.5-02-frontend-stage-split-PLAN.md — Wave 2: Split useAuth.ts into Stage 1 / Stage 2 effects + move onUserInsert/onUpdate into Stage 2 + guestLoginPending state + LoginForm spinner (human-verify checkpoint for UX isolation) (S-03, S-04, D-01, D-03)
- [x] 15.5-03-tests-PLAN.md — Wave 2: Delete VIEW-01 placeholder block from auth-views.test.ts + new auth-subscriptions.test.ts modeling anonymous + authenticated paths (S-05, D-01, D-02)
- [x] 15.5-04-doc-updates-PLAN.md — Wave 3: docs/auth/architecture.md new Subscription Lifecycle section + view retirement; docs/views/architecture.md retirement + rename; docs/FRONTEND-HANDOFF.md rename; docs/roster/contract.md identifier-only rename (D-02, D-04, D-05)
...
- [x] 15.4-02-PLAN.md — Frontend fix-to-compile across ~10 files + tableColumns + BulkUpsert templates (Wave 2, parallel with 03)
- [x] 15.4-03-PLAN.md — Seed pipelines + pairing template + README + harness mirror + local data re-migration checkpoint (Wave 2, parallel with 02)
- [x] 15.4-04-PLAN.md — Test rewrites for 7 files + new synergy auction round-trip + draftMode filter coverage (Wave 3)
...
**Plans:** 17/17 plans complete — COMPLETED 2026-04-18
...
- [x] 15.3-01-PLAN.md — Wave 1 Cluster A: calendar + chat + profile + cursor (7 files, 23 sites)
...
- [x] 15.3-06-PLAN.md — Wave 1 Cluster F: 6 bracket + match-result files (bracketAdvancement audit-only per Pitfall 5)
- [x] 15.3-07-PLAN.md — Wave 1 Cluster G: 10 draft + roster + admin files (highest density, 111 sites)
- [x] 15.3-08-PLAN.md — Wave 2 File 1: finalizationHelpers.ts (14 sites, L148 conditional + P4 carry preserved)
...
- [x] 15.3-15-PLAN.md — Wave 5: non-destructive maincloud publish to `hsrpvp-spacetimedb-nextjs-test1` (identity c2005439f743…d115d6). Empty migration plan (zero schema change). Pre-publish gate rolled into Plan 16 integration run (59 passed + 1 skipped-by-design, 616 tests + 7 todo, exit 0). Post-publish SQL: SYSTEM row intact (id=1, created_by_id=0, last_modified_by_id=0), 359 users, 358 user_identities. Bundle commit fe29995. `.env.local` unmodified. Dashboard https://spacetimedb.com/hsrpvp-spacetimedb-nextjs-test1.
- [x] 15.3-17-PLAN.md — Wave 4c post-publish trailing fix: 4 redundant `as any` eliminated from lobbyLifecycle.ts LobbyMemberAccount inserts (misclassified CORRECT during Plan 16 triage — actually removable per accountSelection.ts L81/L87 precedent). Pure TS annotation removal, zero JS output change, no re-publish needed. Typecheck clean. as-any 189→185 (232 total eliminated, 55.6% of 417 baseline). Commit b60fcfa. PHASE 15.3 COMPLETE 2026-04-18.
...
Plans:
- [x] 15.2-01-PLAN.md — Schema: DeletedUser table + drop User.isPrivate + trim init payloads + drop MyProfileRow.isPrivate (Wave 1)
- [x] 15.2-02-PLAN.md — View flip (anonymousView→view) + R1 bug fix (UserDeletionJob at banAdmin and clientConnected) (Wave 2, parallel with 03)
- [x] 15.2-03-PLAN.md — Cascade refactor: performUserDeletion evicts to DeletedUser + resolveUserLabel helper + 5 call-site migration + nuke flow (Wave 2, parallel with 02)
- [x] 15.2-04-PLAN.md — Publish to maincloud non-destructively + regenerate bindings (server + frontend) + human verify checkpoint (Wave 3)
- [x] 15.2-05-PLAN.md — Tests (D-15/D-16/D-17/D-18/D-19) + docs/auth/architecture.md + docs/views/architecture.md updates (Wave 4)
...
- [x] 15.1-04-PLAN.md — D-18 partial-ingestion regression tests + D-19 guardrail (D-18, D-19)
...
**Plans**: 6 plans
- [x] 16-01-config-route-group-rename-PLAN.md — Next 15.5 bump + top-level typedRoutes + 5-commit route-group rename sequence (FOUND-03, FOUND-04)
- [x] 16-02-subscription-reshuffle-PLAN.md — GameDataProvider 7 useTable + AuthProvider Stage 1 + (authed)/layout.tsx Stage 2 + useAuth.ts trim (FOUND-05, FOUND-06)
...
**Success Criteria** (what must be TRUE):
  ...
  6. `spacetime lock` applied to maincloud production database `hsrpvp-spacetimedb-nextjs-test1` as final operational hardening step (guards against accidental `spacetime delete`).
...
**Plans**: TBD (run `/gsd-plan-phase 16.3` to break down)
