...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.todos)
...
```bash
slug=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query generate-slug "$title" --raw)
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: capture todo - [title]" .planning/todos/pending/[filename] .planning/STATE.md
```
