...
HIGHEST=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phases.list --pick directories[-1])
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(roadmap): add gap closure phases {N}-{M}" .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
