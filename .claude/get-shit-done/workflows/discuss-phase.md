...
AGENT_SKILLS_ADVISOR=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-advisor-researcher)
```
...
if [ -f "D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
