...
**Delegate ROADMAP.md and STATE.md updates to `node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.complete`:**

```bash
TRANSITION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.complete "${current_phase}")
```
...
```text
@D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query progress.bar --raw)
```
...
**Use the transition result from `node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.complete`:**
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query workstream.list --raw)
fi
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set workflow._auto_chain_active false
```
