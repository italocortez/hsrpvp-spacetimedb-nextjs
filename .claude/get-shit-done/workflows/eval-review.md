...
```bash
AUDITOR_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-eval-auditor 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```
...
```markdown
Read D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/agents/gsd-eval-auditor.md for instructions.
