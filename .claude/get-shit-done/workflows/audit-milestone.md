...
```bash
integration_checker_model=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-integration-checker --raw)
```
...
node .claude/get-shit-done/bin/gsd-sdk.cjs query phases.list
```
...
PHASE_INFO=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node .claude/get-shit-done/bin/gsd-sdk.cjs query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
