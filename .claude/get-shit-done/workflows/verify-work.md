...
- Commit via:
  ```bash
  gsd-sdk query commit "docs(phase-${PHASE}): sync ${feature}" --files "docs/${feature}/architecture.md" "docs/${feature}/contract.md"
  ```
...
- Commit via:
  ```bash
  gsd-sdk query commit "test(${PHASE}): record doc skip — ${feature}" --files "{uat_path}"
  ```
