...
```
UI_PHASE_FLAG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" ".planning/phases/XX-name/{phase_num}-UAT.md"
```
