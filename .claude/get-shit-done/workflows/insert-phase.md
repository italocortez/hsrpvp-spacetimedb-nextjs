...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.phase-op "${after_phase}")
...
**Delegate the phase insertion to `node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.insert`:**

```bash
RESULT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.insert "${after_phase}" "${description}")
```
...
1. Update STATE.md's next-phase pointer(s) to the newly inserted phase
   ...
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query state.patch '{"Current Phase":"{decimal_phase}","Next recommended run":"/gsd-plan-phase {decimal_phase}"}'
   ```
...
2. Append a Roadmap Evolution entry via the dedicated handler. It creates the
   ...
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query state.add-roadmap-evolution \
     --phase {decimal_phase} \
     --action inserted \
     --after {after_phase} \
     --note "{description}" \
...
- [ ] `node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.insert` executed successfully
- [ ] Phase directory created
- [ ] Roadmap updated with new phase entry (includes "(INSERTED)" marker)
- [ ] `node .claude/get-shit-done/bin/gsd-sdk.cjs query state.add-roadmap-evolution ...` returned `{ added: true }` or `{ added: false, reason: "duplicate" }`
- [ ] `node .claude/get-shit-done/bin/gsd-sdk.cjs query state.patch` returned matched next-phase pointer field(s)
