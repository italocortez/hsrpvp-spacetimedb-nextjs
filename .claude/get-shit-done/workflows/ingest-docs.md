...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.ingest-docs)
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: ingest {N} docs from {SCAN_PATH} (#2387)" \
  .planning/PROJECT.md \
  .planning/REQUIREMENTS.md \
  .planning/ROADMAP.md \
  .planning/STATE.md \
