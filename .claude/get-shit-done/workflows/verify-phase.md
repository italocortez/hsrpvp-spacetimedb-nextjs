...
<required_reading>
@D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/references/verification-patterns.md
@D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.analyze
```
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `node .claude/get-shit-done/bin/gsd-sdk.cjs query verify.artifacts` (or legacy gsd-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query verify.artifacts "$plan")
...
Use `node .claude/get-shit-done/bin/gsd-sdk.cjs query verify.key-links` (or legacy gsd-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query verify.key-links "$plan")
...
if [ "$GATE_CFG" != "false" ]; then
  ...
  DECISION_RESULT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query check.decision-coverage-verify "${PHASE_DIR}" "${CONTEXT_PATH}")
fi
```
...
See D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/templates/verification-report.md for complete template.
</step>
