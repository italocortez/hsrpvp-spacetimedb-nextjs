...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: map existing codebase" .planning/codebase/*.md
```
