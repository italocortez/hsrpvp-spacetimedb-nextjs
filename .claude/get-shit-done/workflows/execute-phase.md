...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-executor)
```
...
```bash
USE_WORKTREES=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get context_window 2>/dev/null || echo "200000")
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
CROSS_AI_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.cross_ai_execution 2>/dev/null || echo "false")
CROSS_AI_CMD=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.cross_ai_command 2>/dev/null || echo "")
CROSS_AI_TIMEOUT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.cross_ai_timeout 2>/dev/null || echo "300")
```
...
`workflow.cross_ai_command` via `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set workflow.cross_ai_command "<command>"`.
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For `node .claude/get-shit-done/bin/gsd-sdk.cjs query commit` (or legacy `gsd-tools.cjs` commit): add --no-verify flag when needed.
...
5.6. **Post-merge test gate (parallel mode only):**
   ...
   TEST_CMD=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.test_command --default "" 2>/dev/null || true)
   if [ -z "$TEST_CMD" ]; then
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" .planning/ROADMAP.md .planning/STATE.md
     fi
   elif [ "${TEST_EXIT}" -eq 124 ]; then
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node .claude/get-shit-done/bin/gsd-sdk.cjs query verify.key-links {phase_dir}/{plan}-PLAN.md
    ```
...
**Security gate check:**
```bash
SECURITY_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
TDD_MODE=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.tdd_mode 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phase-plan-index "${PHASE_NUMBER}")
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.code_review 2>/dev/null || echo "true")
```
...
```bash
PARENT_INFO=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query find-phase "${PARENT_PHASE}" --raw)
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
# Resolve test command: project config > Makefile > language sniff
REG_TEST_CMD=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.test_command --default "" 2>/dev/null || true)
...
```bash
SCHEMA_DRIFT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query verify.schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-verifier)
```
...
Commit the file:
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "test({phase_num}): persist human verification items as UAT" "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.complete "${PHASE_NUMBER}")
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(phase-{X}): complete phase execution" .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
**Check config gate:**
```bash
GL_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
if [ ${#CLOSED[@]} -gt 0 ]; then
  node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(phase-${PHASE_NUMBER}): auto-close ${#CLOSED[@]} todo(s) resolved by this phase" .planning/todos/completed/ .planning/STATE.md || true
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(phase-{X}): evolve PROJECT.md after phase completion" .planning/PROJECT.md
```
