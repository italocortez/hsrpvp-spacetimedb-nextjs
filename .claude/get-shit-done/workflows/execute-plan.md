...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.execute-phase "${PHASE}")
...
# config settings can be fetched via node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get if needed
```
...
INLINE_THRESHOLD=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query phases.list --type summaries --raw
...
```bash
NODE_REPAIR=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
Update STATE.md using node .claude/get-shit-done/bin/gsd-sdk.cjs query (or legacy gsd-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  node .claude/get-shit-done/bin/gsd-sdk.cjs query state.advance-plan

  # Recalculate progress bar from disk state
  node .claude/get-shit-done/bin/gsd-sdk.cjs query state.update-progress

  # Record execution metrics
  node .claude/get-shit-done/bin/gsd-sdk.cjs query state.record-metric \
...
node .claude/get-shit-done/bin/gsd-sdk.cjs query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node .claude/get-shit-done/bin/gsd-sdk.cjs query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using node .claude/get-shit-done/bin/gsd-sdk.cjs query (or legacy gsd-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs({phase}-{plan}): complete [plan-name] plan" .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs({phase}-{plan}): complete [plan-name] plan" .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
