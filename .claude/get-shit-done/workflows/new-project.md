...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-roadmapper 2>/dev/null)
```
...
```bash
mkdir -p .planning
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-new-project '{"mode":"yolo","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":true|false,"auto_advance":true}}'
```
...
```bash
mkdir -p .planning
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "chore: add project config" .planning/config.json
```
...
```bash
mkdir -p .planning
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: initialize project" .planning/PROJECT.md
```
...
```bash
mkdir -p .planning
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-new-project '{"mode":"[yolo|interactive]","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":[false if granularity=coarse, true otherwise]}}'
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: define v1 requirements" .planning/REQUIREMENTS.md
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query generate-claude-md --output "$INSTRUCTION_FILE"
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: create roadmap ([N] phases)" .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
```bash
PHASE1_SECTION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase 1 2>/dev/null)
