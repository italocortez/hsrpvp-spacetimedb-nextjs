...
```bash
GRADUATION_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
