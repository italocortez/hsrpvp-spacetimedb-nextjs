...
**Safe merge:** Apply each chosen value via `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set <key.path> <value>` so unrelated keys are never clobbered. `code_review_depth` is written only if the code_review question was answered `on`; otherwise leave the existing value in place.
