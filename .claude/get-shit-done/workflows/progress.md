...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.progress)
...
<step name="load">
**Use structured extraction from `node .claude/get-shit-done/bin/gsd-sdk.cjs query` (or legacy gsd-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.analyze)`
- `STATE=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node .claude/get-shit-done/bin/gsd-sdk.cjs query summary-extract <path> --fields one_liner
  ```
...
<step name="report">
**Generate progress bar from `node .claude/get-shit-done/bin/gsd-sdk.cjs query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query progress.bar --raw)
```
...
```bash
DEBT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "$((Z+1))" 2>/dev/null)
