...
global: `D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
...
even when custom files exist (bug #1997). Use `node .claude/get-shit-done/bin/gsd-sdk.cjs query detect-custom-files`
...
if [ -n "$RUNTIME_DIR" ] && command -v gsd-sdk >/dev/null 2>&1; then
  CUSTOM_JSON=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
