...
Apply each selected value via `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set <key> <value>` so the central
...
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set workflow.plan_bounce_passes 5
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set workflow.subagent_timeout 900
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set git.base_branch main
node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set context_window 1000000
```
...
route each write through `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set` so sibling preservation is handled by
the central setter.
</step>
...
- [ ] Writes routed through `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set` so unrelated keys are preserved
