...
```bash
CONVERGENCE_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```
...
```bash
node "D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
