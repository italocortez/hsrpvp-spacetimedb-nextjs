...
```bash
AGENT_SKILLS_DEBUGGER=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-debugger 2>/dev/null)
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs({phase_num}): add root causes from diagnosis" ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
