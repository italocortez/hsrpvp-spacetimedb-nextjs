...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.plan-phase "$PHASE")
...
```bash
CHUNKED_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
```bash
PHASE_INFO=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${PHASE}")
```
...
5. Commit:
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(${padded_phase}): generate context from PRD" "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
AI_PHASE_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
```bash
PHASE_DESC=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${PHASE}" --pick section)
```
...
SECURITY_ASVS=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${PHASE}" 2>/dev/null)
...
```bash
AUTO_CHAIN=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
```bash
PHASE_SECTION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set workflow.nyquist_validation false`
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.pattern_mapper 2>/dev/null || echo "true")
```
...
5. **Commit per-plan:**
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "refactor(${padded_phase}): bounce plans through external refinement" "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
POST_PLANNING_GAPS=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.post_planning_gaps --default true 2>/dev/null || echo true)
