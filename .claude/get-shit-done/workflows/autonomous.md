...
```bash
DETAIL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
Read and execute: `D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/references/autonomous-smart-discuss.md`
