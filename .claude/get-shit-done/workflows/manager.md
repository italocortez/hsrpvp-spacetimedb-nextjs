...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.manager)
...
These are empty strings by default. Set via: `node .claude/get-shit-done/bin/gsd-sdk.cjs query config-set manager.flags.discuss "--auto --analyze"`
