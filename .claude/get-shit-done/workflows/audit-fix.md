...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query audit-uat 2>/dev/null || echo "{}")
...
**b. Run tests:**
```bash
AUDIT_TEST_CMD=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.test_command --default "" 2>/dev/null || true)
