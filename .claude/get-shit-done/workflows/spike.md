...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(spike-NNN): [VERDICT] — [key finding]" .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(spikes): update conventions" .planning/spikes/CONVENTIONS.md
```
</step>
