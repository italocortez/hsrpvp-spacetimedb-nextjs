...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query audit-open
```
...
1. Re-run `node .claude/get-shit-done/bin/gsd-sdk.cjs query audit-open --json` to get structured data
...
```bash
ROADMAP=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node .claude/get-shit-done/bin/gsd-sdk.cjs query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `node .claude/get-shit-done/bin/gsd-sdk.cjs query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `node .claude/get-shit-done/bin/gsd-sdk.cjs query milestone.complete`:**

```bash
ARCHIVE=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "chore: archive v[X.Y] milestone files" .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**If doesn't exist:** Create from template at `D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/templates/retrospective.md`.
...
**Commit:**
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: update retrospective for v${VERSION}" .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query init.execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get git.base_branch 2>/dev/null || echo "")
