...
```bash
SELECTOR_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
