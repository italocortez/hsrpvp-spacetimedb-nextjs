...
AGENT_SKILLS_UI=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query agent-skills gsd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs(${padded_phase}): UI design contract" "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
