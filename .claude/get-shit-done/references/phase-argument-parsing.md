...
```bash
PHASE_INFO=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query find-phase "${PHASE}" --raw)
```
