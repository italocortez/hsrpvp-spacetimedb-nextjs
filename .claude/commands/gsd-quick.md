...
<execution_context>
@D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.get .planning/quick/{dir}/SUMMARY.md status 2>/dev/null
  ```
...
5. Load context via:
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query init.quick
   ```
...
Execute the quick workflow from @D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/workflows/quick.md end-to-end.
