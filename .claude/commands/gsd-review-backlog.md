...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query phase.add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: review backlog — promoted N, removed M" .planning/ROADMAP.md
   ```
