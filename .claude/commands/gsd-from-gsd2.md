...
**CJS-only:** `from-gsd2` is not on the `node .claude/get-shit-done/bin/gsd-sdk.cjs query` registry; call `gsd-tools.cjs` as shown below (see `docs/CLI-TOOLS.md`).
</objective>
...
1. **Locate the .gsd/ directory** — check the current working directory (or `--path` argument):
   ```bash
   node "D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2 --dry-run
   ```
...
3. **Run the migration** after confirmation:
   ```bash
   node "D:/GitsWork/hsrpvp-spacetimedb-nextjs/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2
   ```
