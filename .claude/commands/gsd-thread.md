...
- Read frontmatter `status` field via:
  ```bash
  node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.get .planning/threads/{file} status 2>/dev/null
  ```
...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.set .planning/threads/{SLUG}.md status resolved
   node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```

3. Commit:
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: resolve thread — {SLUG}" ".planning/threads/{SLUG}.md"
   ```
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.set .planning/threads/{SLUG}.md status in_progress
node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
...
1. Generate slug from description:
   ```bash
   SLUG=$(node .claude/get-shit-done/bin/gsd-sdk.cjs query generate-slug "$ARGUMENTS" --raw)
   ```
...
5. Commit:
   ```bash
   node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs: create thread — ${ARGUMENTS}" ".planning/threads/${SLUG}.md"
   ```
...
- Status fields read via node .claude/get-shit-done/bin/gsd-sdk.cjs query frontmatter.get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through node .claude/get-shit-done/bin/gsd-sdk.cjs query (or gsd-tools) which sanitizes input — keep that pattern
</security_notes>
