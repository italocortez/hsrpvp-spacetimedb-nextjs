...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
for decision in "${DECISIONS[@]}"; do
  node .claude/get-shit-done/bin/gsd-sdk.cjs query state.add-decision "${decision}"
done
...
node .claude/get-shit-done/bin/gsd-sdk.cjs query roadmap.update-plan-progress "${PHASE_NUMBER}"
...
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query state.add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
node .claude/get-shit-done/bin/gsd-sdk.cjs query commit "docs({phase}-{plan}): complete [plan-name] plan" \
