...
2. Player calls request_join_team → TournamentTeamRequest inserted (isPending=true)

3. Captain calls accept_team_request → request.isPending=false
   → Player's TournamentParticipant updated to teamGroupId=teamId, type=Team

   OR

3. Captain calls reject_team_request → request row deleted entirely
