...
export const admin_delete_row = spacetimedb.reducer({ tableName: t.string(), primaryKeyJson: t.string() }, (ctx, { tableName, primaryKeyJson }) => {
    ...
    switch (tableName) {
        case 'User': {
            ...
            if (!user)
                throw new SenderError('Row not found');
            // Already pending deletion
            if (user.deletedAt)
                throw new SenderError(`User #${id} is already pending deletion.`);
            ...
            if (hostedLobbies.length > 0) {
                throw new SenderError(`Cannot delete user #${id}: they are hosting lobby "${hostedLobbies[0].joinCode}". Remove the lobby first.`);
            }
            ...
            if (memberships.length > 0) {
                throw new SenderError(`Cannot delete user #${id}: they are in active lobby #${memberships[0].lobbyId}. Remove them from the lobby first.`);
            }
            // Block deletion if user is in an active match step
            for (const step of ctx.db.MatchSessionStep.iter()) {
                if (step.actorUserId === id) {
                    throw new SenderError(`Cannot delete user #${id}: they have actions in active match (lobby #${step.lobbyId}). End the match first.`);
                }
            }
            ...
            const deleteAt = ctx.timestamp.microsSinceUnixEpoch + 5000000n;
        ...
        case 'HsrCharacter': {
            if (!ctx.db.HsrCharacter.name.find(primaryKeyJson))
                throw new SenderError('Row not found');
            ctx.db.HsrCharacter.name.delete(primaryKeyJson);
            break;
        }
        case 'HsrLightcone': {
            if (!ctx.db.HsrLightcone.name.find(primaryKeyJson))
                throw new SenderError('Row not found');
        ...
        case 'HsrSynergyCost': {
            const id = Number(primaryKeyJson);
            if (!ctx.db.HsrSynergyCost.id.find(id))
                throw new SenderError('Row not found');
        ...
        case 'LobbyMember': {
            const key = JSON.parse(primaryKeyJson);
            const memberTable = ctx.db.LobbyMember;
        ...
        case 'MatchSession': {
            const id = Number(primaryKeyJson);
            if (!ctx.db.MatchSession.lobbyId.find(id))
                throw new SenderError('Row not found');
        ...
        case 'MatchSessionStep': {
            const id = Number(primaryKeyJson);
            if (!ctx.db.MatchSessionStep.id.find(id))
                throw new SenderError('Row not found');
            ctx.db.MatchSessionStep.id.delete(id);
            break;
        }
        case 'MatchSessionHistory': {
            if (!ctx.db.MatchSessionHistory.id.find(primaryKeyJson))
                throw new SenderError('Row not found');
            ctx.db.MatchSessionHistory.id.delete(primaryKeyJson);
            break;
        }
        case 'MatchSessionStepHistory': {
            if (!ctx.db.MatchSessionStepHistory.matchId.find(primaryKeyJson))
                throw new SenderError('Row not found');
...
export const admin_bulk_upsert = spacetimedb.reducer({ tableName: t.string(), jsonData: t.string() }, (ctx, { tableName, jsonData }) => {
    const admin = ensureAdmin(ctx);
    const rows = JSON.parse(jsonData);
    if (!Array.isArray(rows))
        throw new SenderError('jsonData must be a JSON array');
    ...
    switch (tableName) {
        case 'HsrCharacter': {
            for (const r of rows) {
                ...
                if (existing) {
                    ctx.db.HsrCharacter.name.update({ ...existing, ...row, ...auditUpdate(ctx, existing, admin.id) });
                }
                else {
                    ctx.db.HsrCharacter.insert({ ...row, ...auditInsert(ctx, admin.id) });
                }
            }
            break;
        }
        case 'HsrLightcone': {
            for (const r of rows) {
                ...
                if (existing) {
                    ctx.db.HsrLightcone.name.update({ ...existing, ...row, ...auditUpdate(ctx, existing, admin.id) });
                }
                else {
                    ctx.db.HsrLightcone.insert({ ...row, ...auditInsert(ctx, admin.id) });
                }
            }
            break;
        }
        ...
        case 'HsrLightconeCost': {
            for (const r of rows) {
                ...
                const row = {
                    ...
                    gameMode: { tag: r.gameMode, value: undefined },
                    classicCosts: r.classicCosts,
                    auctionBaseBid: r.auctionBaseBid,
                };
                if (existing) {
...
export const admin_update_user = spacetimedb.reducer({ userId: t.u32(), displayName: t.string(), username: t.string(), roleTag: t.string() }, (ctx, { userId, displayName, username, roleTag }) => {
    const admin = ensureAdmin(ctx);
