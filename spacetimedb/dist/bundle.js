...
spacetimedb.clientConnected((ctx) => {
	...
	if (existingUser) ctx.db.User.identity.update({
		...existingUser,
		lastLoginAt: ctx.timestamp
	});
	else {
		const guestName = `Guest-${ctx.sender.toHexString().substring(0, 8)}`;
		ctx.db.User.insert({
			identity: ctx.sender,
			username: guestName,
